...
function TopNav() {
  ...
  console.log(results, "results from navbar");
  ...
  }, [debouncedQuery, query, handleSearch, dispatch]);

  const handleShow = () => setSearchModal(true);
  const handleClose = () => setSearchModal(false);
  useEffect(() => {
    if (location.state?.fromSearchModal) {
      // Reopen the modal if the user came from the search modal
      setSearchModal(true);
    }
  }, [location.state]);
